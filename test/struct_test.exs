...
defmodule Struct.Test do
  ...
  test "can transform nested schema" do
    assert %{
             ...
             |> NestedStruct.struct ==
             %NestedStruct{id: "test",
                           value: "value",
                           nested: %TestStruct{id: "nested",
                                               value: "nested value"}}
  end
  ...
  test "object id" do
    assert ObjectIdStruct.struct(%{
      id: BSON.ObjectId.new(1,1,1,1)
    }) ==  %ObjectIdStruct{
      id: BSON.ObjectId.new(1,1,1,1)
    }
    assert ObjectIdStruct.struct(%{
      id: BSON.ObjectId.encode!(BSON.ObjectId.new(1,1,1,1))
    }) ==  %ObjectIdStruct{
      id: BSON.ObjectId.new(1,1,1,1)
    }
  end
